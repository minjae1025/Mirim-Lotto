...
class Client extends Save_Sys{
	...
	void rankPrint(int num) throws InterruptedException {
		...
		for (int i = 0; i<num; i++) {
			...
			week_money += getPrizeAmount(prize_rank[i]);
		}
		
		if (week_money > 0) {
			System.out.println("축하드립니다! 상금 "+week_money+"원 지금되었습니다!");
			money += week_money;
		...
		if (num <= 5 && num > 0) {
			...
			if (menu == 1) {
				...
				for (int i = 0; i<num; i++) {
					...
					for (int j = 0; j<my_lotto[i].length; j++) {
						System.out.printf(" |%2d| ", my_lotto[i][j]);
					}
					System.out.println();
				}
				...
				for (int i = 0; i<num; i++) {
					my_lotto[i] = LottoNumberGUI.inputNum();
					System.out.print((i+1)+"번째 장 번호 :");
					for (int j = 0; j<my_lotto[i].length; j++) {
						System.out.printf(" |%2d| ", my_lotto[i][j]);
					}
